...
public class Registration extends JFrame {
	...
	public Registration() {
		...
		lblRegistration.setFont(new Font("Verdana", Font.BOLD, 31));
		...
		JLabel lblWelcome = new JLabel("Click to Change Profile");
		...
		txtUserame.addFocusListener(new FocusAdapter() {
			...
			public void focusLost(FocusEvent arg0) {
				...
				if(foundUser) {
					...
					JOptionPane.showMessageDialog(null, Username+" already exits choose another");
				}else {
		...
		JLabel lblWeightGoal = new JLabel("Height(in feet)");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				double WeigtGoal = Double.parseDouble(txtWeightGoal.getText());
				String unit = cbUnit.getSelectedItem().toString();
				if(unit.equals("lb")) {
					WeigtGoal = convertPoundsToKilogram(WeigtGoal);
				}
				User user = new User(Username,Password,Name,Image,DOB,Gender,Height,WeigtGoal);
				foundUser = helper.foundUser(Username);
				if(foundUser) {
					txtUserame.setBorder(new MatteBorder(2, 2, 2, 2, (Color) new Color(255, 0, 0)));
					JOptionPane.showMessageDialog(null, Username+" already exits choose another");
